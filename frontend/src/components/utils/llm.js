...
不需要使用多余的前缀，直接给出解释。你的回答需要尽量简要，体现出这个视觉元素与其他视觉元素的差异。
你的输出需要以json格式给出。按照以下格式:
{
  "content": "视觉元素的解释"
}`;
...
const defaultBoxPrompt = `对于下面的信息图，请结合主题和信息图描述，逐个解释每个红色框中的元素。
每个红色框中都包含一个数字标号。请按照标号顺序，逐一说明每个红色框中的内容，提供该元素的解释，并简明扼要地描述它们在信息图中的作用。 
你的输出需要以json格式给出。按照以下格式:
{
  "elements": [
    { "content": "元素0的解释" }, // 元素0的解释
    { "content": "元素1的解释" }, // 元素1的解释
  ]
}`;
*/
